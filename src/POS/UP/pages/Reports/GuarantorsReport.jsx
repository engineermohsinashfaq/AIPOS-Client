...
// Main GuarantorsReports component function
export default function GuarantorsReport() {
  ...
  const stats = useMemo(() => {
    ...
    const withAddress = filteredGuarantors.filter(g => g.address && g.address.trim() !== '').length;
  ...
  const initials = (g) =>
    `${(g.firstName || "").charAt(0)}${(g.lastName || "").charAt(0)}`.toUpperCase();
  ...
  const saveChanges = () => {
    ...
    setGuarantors((prev) =>
      prev.map((g) =>
        g.guarantorId === form.guarantorId ? updatedForm : g
      )
    );
  ...
  const handleDownloadReport = () => {
    ...
    const success = exportToExcel(filteredGuarantors, `guarantors-report-${new Date().toISOString().split('T')[0]}`);
    if (success) {
      notifySuccess("GUARANTORS REPORT EXPORTED TO EXCEL SUCCESSFULLY");
    }
  };
  ...
  return (
    ...
    <div className="p-2 min-h-screen text-white">
      ...
      <div className="max-w-8xl mx-auto space-y-6">
        {/* Page header section */}
        <div>
          <h1 className="text-3xl font-bold mb-2">GUARANTORS REPORTS</h1>
          <p className="text-white/80">
            ANALYZE AND EXPORT GUARANTORS DATA WITH ADVANCED FILTERING AND REPORTING.
          </p>
        </div>
        ...
        <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-md p-4 grid grid-cols-1 md:grid-cols-5 gap-3">
          ...
          <div className="flex items-center gap-2 justify-between">
            ...
            >
              <option value="All" className="bg-black/95 text-white">ALL</option>
              {uniqueCities.map(city => (
                <option key={city} value={city} className="bg-black/95 text-white">
                  {city}
                </option>
              ))}
            </select>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="bg-green-100 border border-green-300 rounded-lg p-4 text-center">
                  <div className="text-2xl font-bold text-green-900">{stats.withAddress}</div>
                ...
                <div className="bg-purple-100 border border-purple-300 rounded-lg p-4 text-center">
                  <div className="text-2xl font-bold text-purple-900">{stats.recentlyAdded}</div>
              ...
              {uniqueCities.length > 0 && (
                <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                  <h3 className="font-bold text-gray-800 mb-3">CITY DISTRIBUTION</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    {uniqueCities.slice(0, 6).map(city => {
                      const cityCount = filteredGuarantors.filter(g => g.city === city).length;
                      return (
